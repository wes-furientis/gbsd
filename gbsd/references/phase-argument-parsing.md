...
```bash
PHASE_INFO=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" find-phase "${PHASE}" --raw)
```
