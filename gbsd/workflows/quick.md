<purpose>
Execute small, ad-hoc tasks with GBSD guarantees (atomic commits, STATE.md tracking). Quick mode spawns gbsd-planner (quick mode) + gbsd-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" init quick "$DESCRIPTION")
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/gbsd:new-project` first.
...
Spawn gbsd-executor with plan reference:
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
GBSD > QUICK TASK COMPLETE (FULL MODE)
...
Ready for next task: /gbsd:quick
```
