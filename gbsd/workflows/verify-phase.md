...
<required_reading>
@~/.claude/gbsd/references/verification-patterns.md
@~/.claude/gbsd/templates/verification-report.md
</required_reading>
...
Use gbsd-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use gbsd-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" verify artifacts "$plan")
...
Use gbsd-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" verify key-links "$plan")
...
See ~/.claude/gbsd/templates/verification-report.md for complete template.
</step>
