...
**Delegate ROADMAP.md and STATE.md updates to gbsd-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gbsd-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `gbsd-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/gbsd:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/gbsd:discuss-phase [X+1] --auto")
...
`/gbsd:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/gbsd:plan-phase [X+1]` — skip discussion, plan directly
- `/gbsd:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/gbsd:discuss-phase [X+1]` — revisit context
- `/gbsd:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/gbsd:complete-milestone {version}")
