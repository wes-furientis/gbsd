...
```bash
INIT=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" resolve-model gbsd-integration-checker --raw)
```
...
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="gbsd-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gbsd:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gbsd:validate-phase {N}` |

Phases needing validation: run `/gbsd:validate-phase {N}` for each flagged phase.
...
- /gbsd:complete-milestone {version} — proceed anyway (accept tech debt)
