<purpose>
Create all phases necessary to close gaps identified by `/gbsd:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gbsd:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gbsd:audit-milestone` first.
```
...
PHASES=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gbsd:execute-phase {N}` — if plans already exist
...
`/gbsd:audit-milestone` — re-audit to verify gaps closed
`/gbsd:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gbsd:plan-phase` next
</success_criteria>
