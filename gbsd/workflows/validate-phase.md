...
```bash
AUDITOR_MODEL=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" resolve-model gbsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" config get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /gbsd:settings."

Display banner: `GBSD > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /gbsd:execute-phase {N} first."
...
## 5. Spawn gbsd-nyquist-auditor

```
Task(
  prompt="Read ~/.claude/agents/gbsd-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>",
  subagent_type="gbsd-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/gbsd/templates/VALIDATION.md`
...
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
GBSD > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /gbsd:audit-milestone
```

**Partial:**
```
GBSD > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /gbsd:validate-phase {N}
```
