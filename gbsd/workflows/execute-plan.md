...
```bash
INIT=$(node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" init execute-phase "${PHASE}")
...
# config settings can be fetched via gbsd-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gbsd-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gbsd-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/gbsd/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/gbsd/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/gbsd/bin/gbsd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/gbsd/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/gbsd/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/gbsd/templates/summary.md`.
...
Update STATE.md using gbsd-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state record-metric \
...
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gbsd-tools:

```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gbsd:execute-phase {phase}` + `/gbsd:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gbsd:plan-phase {Z+1}` + `/gbsd:verify-work {Z}` + `/gbsd:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gbsd:complete-milestone` + `/gbsd:verify-work` + `/gbsd:add-phase` |
