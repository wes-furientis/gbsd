...
1. **gbsd-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **gbsd-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/gbsd:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /gbsd:progress to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /gbsd:plan-phase {X} to replan
...
**Also available:**
- `/gbsd:plan-phase ${PHASE} --skip-research` — plan without research
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "$HOME/.claude/gbsd/bin/gbsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /gbsd:execute-phase ${PHASE}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /gbsd:plan-phase ${PHASE}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /gbsd:plan-phase ${PHASE} --gaps
  ```
