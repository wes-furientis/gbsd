...
> **Note:** Planning methodology is in `agents/gbsd-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/gbsd/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/gbsd:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/gbsd/references/tdd.md` for TDD plan structure.
...
See `~/.claude/gbsd/templates/user-setup.md` for full schema and examples
...
See `~/.claude/gbsd/workflows/verify-phase.md` for verification logic.
