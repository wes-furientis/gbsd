...
describe('resolveModelInternal', () => {
  ...
  describe('model profile structural validation', () => {
    test('all known agents resolve to a valid string for each profile', () => {
      const knownAgents = ['gbsd-planner', 'gbsd-executor', 'gbsd-phase-researcher', 'gbsd-codebase-mapper'];
  ...
  describe('override precedence', () => {
    test('per-agent override takes precedence over profile', () => {
      writeConfig({
        ...
        model_overrides: { 'gbsd-executor': 'haiku' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gbsd-executor'), 'haiku');
    });

    test('opus override resolves to inherit', () => {
      writeConfig({
        model_overrides: { 'gbsd-executor': 'opus' },
      });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gbsd-executor'), 'inherit');
    });
    ...
    test('agents not in override fall back to profile', () => {
      ...
      // gbsd-planner not overridden, should use quality profile -> opus -> inherit
      assert.strictEqual(resolveModelInternal(tmpDir, 'gbsd-planner'), 'inherit');
    });
  });
  ...
  describe('edge cases', () => {
    test('returns sonnet for unknown agent type', () => {
      writeConfig({ model_profile: 'balanced' });
      assert.strictEqual(resolveModelInternal(tmpDir, 'gbsd-nonexistent'), 'sonnet');
    });

    test('defaults to balanced profile when model_profile missing', () => {
      writeConfig({});
      // balanced profile, gbsd-planner -> opus -> inherit
      assert.strictEqual(resolveModelInternal(tmpDir, 'gbsd-planner'), 'inherit');
    });
  });
});
