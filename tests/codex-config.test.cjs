/**
 * GBSD Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('gbsd-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('gbsd-plan-phase');
    assert.ok(result.includes('`$gbsd-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{GBSD_ARGS}}'), 'has GBSD_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('gbsd-discuss-phase');
...
describe('convertClaudeAgentToCodexAgent', () => {
  test('adds codex_agent_role header and cleans frontmatter', () => {
    const input = `---
name: gbsd-executor
description: Executes GBSD plans with atomic commits
tools: Read, Write, Edit, Bash, Grep, Glob
color: yellow
---

<role>
You are a GBSD plan executor.
</role>`;
    ...
    assert.ok(result.includes('"gbsd-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes GBSD plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: gbsd-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes GBSD plans with atomic commits'), 'purpose from description');
...
Run /gbsd:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$gbsd-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/gbsd:execute-phase'), 'original slash command removed');
  });
...
<role>You check plans.</role>`;
  ...
  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('gbsd-unknown', sampleAgent);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'gbsd-executor', 'gbsd-planner', 'gbsd-phase-researcher',
      'gbsd-project-researcher', 'gbsd-research-synthesizer', 'gbsd-verifier',
      'gbsd-codebase-mapper', 'gbsd-roadmapper', 'gbsd-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['gbsd-plan-checker', 'gbsd-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'gbsd-executor', description: 'Executes plans' },
    { name: 'gbsd-planner', description: 'Creates plans' },
  ];

  test('starts with GBSD marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(GBSD_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('includes per-agent sections', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.gbsd-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.gbsd-planner]'), 'has planner section');
    assert.ok(result.includes('config_file = "agents/gbsd-executor.toml"'), 'has executor config_file');
...
describe('stripGbsdFromCodexConfig', () => {
  test('returns null for GBSD-only config', () => {
    const content = `${GBSD_CODEX_MARKER}\n[features]\nmulti_agent = true\n`;
    const result = stripGbsdFromCodexConfig(content);
    assert.strictEqual(result, null, 'returns null when GBSD-only');
  });
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes('multi_agent'), 'removes GBSD content');
    assert.ok(!result.includes(GBSD_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(GBSD_CODEX_MARKER), 'strips marker');
  });

  test('removes [agents.gbsd-*] sections', () => {
    const content = `[agents.gbsd-executor]\ndescription = "test"\nconfig_file = "agents/gbsd-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripGbsdFromCodexConfig(content);
    assert.ok(!result.includes('[agents.gbsd-executor]'), 'removes GBSD agent section');
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gbsd-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(GBSD_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('multi_agent = true'), 'has feature flag');
    assert.ok(content.includes('[agents.gbsd-executor]'), 'has agent');
  });

  test('case 2: replaces existing GBSD block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'gbsd-executor', description: 'Updated description' },
      { name: 'gbsd-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.gbsd-planner]'), 'has new agent');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(GBSD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without GBSD marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(GBSD_CODEX_MARKER), 'adds marker');
  ...
  test('case 3 with existing [features]: injects keys', () => {
    ...
    assert.ok(content.includes(GBSD_CODEX_MARKER), 'adds marker for agents block');
  });
  ...
  test('case 2 after case 3 with existing [features]: no duplicate sections', () => {
    ...
    assert.ok(content.includes('multi_agent = true'), 'has GBSD feature key');
    assert.ok(content.includes('[agents.gbsd-executor]'), 'has agent');
  });
  ...
  test('case 2 strips leaked [agents] from before content', () => {
    ...
    assert.ok(content.includes('[agents.gbsd-executor]'), 'has agent from fresh block');
  });
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'gbsd-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes('[agents.gbsd-executor]'), 'has executor agent');

    // Verify per-agent .toml files
    const agentsDir = path.join(tmpTarget, 'agents');
    assert.ok(fs.existsSync(path.join(agentsDir, 'gbsd-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'gbsd-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'gbsd-executor.toml'), 'utf8');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'gbsd-plan-checker.toml'), 'utf8');
