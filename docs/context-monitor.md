...
| CRITICAL | <= 25% | Stop immediately, save state (`/gbsd:pause-work`) |
...
```
Statusline Hook (gbsd-statusline.js)
    | writes
    v
/tmp/claude-ctx-{session_id}.json
    ^ reads
    |
Context Monitor (gbsd-context-monitor.js, PostToolUse/AfterTool)
    | injects
    v
...
## Integration with GBSD

GBSD's `/gbsd:pause-work` command saves execution state. The WARNING message suggests using it. The CRITICAL message instructs immediate state save.

## Setup

Both hooks are automatically registered during `npx @wes-furientis/gbsd` installation:
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/gbsd-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/gbsd-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/gbsd-context-monitor.js"
          }
        ]
      }
    ]
